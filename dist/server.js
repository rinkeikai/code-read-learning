...
export function createServer() {
    ...
    server.registerTool("list_modules", {
        description: "学習対象プロジェクト内のモジュール一覧を返す。親リポジトリ（root）と Git サブモジュールを列挙する。",
        inputSchema: {},
    }, async () => {
    ...
    server.registerTool("get_learning_material", {
        description: "Git 差分からコード読解学習用の教材を生成する。ステージ済み変更を優先し、なければ最新コミット (HEAD) を利用する。サブモジュールがある場合は module で対象を指定する。コード解説は行わず、教材データのみ返す。",
        inputSchema: {
            module: z
                .string()
                .optional()
